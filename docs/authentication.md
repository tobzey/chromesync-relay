...
A post-approval store failure records `STORE_UNAVAILABLE` when storage can be written and retries execution once after a short delay without rejecting the committed decision. If the unused grant expires, **Review and retry** requires a fresh approval. Startup resumes approvals independently.
